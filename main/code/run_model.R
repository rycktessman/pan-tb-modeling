...
#load parameters
#setwd("~/GitHub/pan-tb-model/")
load("data/params.Rda")
load("data/regimen_params.Rda")
load("data/cost_params.Rda")
#load("data/cost_params_LAI.Rda") #we no longer have separate LAI cost params
...
n <- 10000 #number of parameter set samples
tic()

for(analysis in analyses) {
  print(analysis)
  if(analysis %in% c("bad_efficacy", "good_efficacy", "bad_duration", "good_duration", "bad_adherence", "good_adherence",
                     "bad_forgiveness", "good_forgiveness", "bad_safety", "good_safety",
                     "bad_baseres", "good_baseres", "bad_restrend", "good_restrend")) {
    scenarios <- c("SQ", "Pan", "Pan_inject")
  } else {
    scenarios <- names(reg_params[[country]])
  }
  #OWSA - only run SQ, Pan, & Pan_inject; vary parameters
  if(analysis=="bad_efficacy") {
    reg_params[[country]][["Pan"]]$efficacy_P <- rep(0.9, n)
    reg_params[[country]][["Pan_inject"]]$efficacy_P <- rep(0.9, n)
  } else if(analysis=="good_efficacy") {
    reg_params[[country]][["Pan"]]$efficacy_P <- rep(0.99, n)
    reg_params[[country]][["Pan_inject"]]$efficacy_P <- rep(0.99, n)
  } else if(analysis=="good_duration") { #affects LAI monitoring costs. Costs update automatically with shorter duration
    reg_params[[country]][["Pan"]]$dur_P <- 2*4
    reg_params[[country]][["Pan_inject"]]$dur_P <- 2*4
  } else if(analysis=="bad_duration") { #affects LAI monitoring costs. Costs don't update automatically with longer duration
    reg_params[[country]][["Pan"]]$dur_P <- reg_params[[country]][["Pan"]]$dur_RS
    reg_params[[country]][["Pan_inject"]]$dur_P <- reg_params[[country]][["Pan"]]$dur_RS
    cost_params[[country]]$outpatient_sched[ind_P,] <- cost_params[[country]]$outpatient_sched[ind_RS,]
    cost_params[[country]]$smear_sched[ind_P,] <- cost_params[[country]]$smear_sched[ind_RS,]
    cost_params[[country]]$culture_sched[ind_P,] <- cost_params[[country]]$culture_sched[ind_RS,]
    cost_params[[country]]$cxr_sched[ind_P,] <- cost_params[[country]]$cxr_sched[ind_RS,]
    cost_params[[country]]$livertest_sched[ind_P,] <- cost_params[[country]]$livertest_sched[ind_RS,]
    cost_params[[country]]$fullblood_sched[ind_P,] <- cost_params[[country]]$fullblood_sched[ind_RS,]
    cost_params[[country]]$ecg_sched[ind_P,] <- cost_params[[country]]$ecg_sched[ind_RS,]
    cost_params[[country]]$neuroscreen_sched[ind_P,] <- cost_params[[country]]$neuroscreen_sched[ind_RS,]
    cost_params[[country]]$support_sched <- lapply(1:n, function(x)
      replace_schedule(cost_params[[country]]$support_sched[[x]], ind_P, ind_RS))
    cost_params[[country]]$ooptx_sched <- lapply(1:n, function(x)
      replace_schedule(cost_params[[country]]$ooptx_sched[[x]], ind_P, ind_RS))
    cost_params[[country]]$indirecttx_sched <- lapply(1:n, function(x)
      replace_schedule(cost_params[[country]]$indirecttx_sched[[x]], ind_P, ind_RS))
  } else if(analysis=="bad_adherence") { #doesn't affect LAI
    reg_params[[country]][["Pan"]]$adherence_P <- cbind(rep(0, n),
                                                        rep(0.25, n),
                                                        rep(0.15, n),
                                                        rep(0.25, n),
                                                        rep(0.35, n))
  } else if(analysis=="good_adherence") { #doesn't affect LAI
    reg_params[[country]][["Pan"]]$adherence_P <- cbind(rep(0, n),
                                                        rep(0.05, n),
                                                        rep(0.15, n),
                                                        rep(0.13, n),
                                                        rep(0.67, n))
  } else if(analysis=="bad_forgiveness") { #doesn't affect LAI
    reg_params[[country]][["Pan"]]$forgive_P <- rep(0.1, n)
  } else if(analysis=="good_forgiveness") { #doesn't affect LAI
    reg_params[[country]][["Pan"]]$forgive_P <- rep(0.3, n)
  } else if(analysis=="bad_safety") {
    dur_P <- reg_params[[country]]$Pan$dur_P
    cost_params[[country]]$p_liverAE[, ind_P] <- cost_params[[country]]$p_liverAE[, ind_RR]
    cost_params[[country]]$p_pancreasAE[, ind_P] <- cost_params[[country]]$p_pancreasAE[, ind_RR]
    cost_params[[country]]$p_anemiaAE[, ind_P] <- cost_params[[country]]$p_anemiaAE[, ind_RR]
    cost_params[[country]]$p_neutropeniaAE[, ind_P] <- cost_params[[country]]$p_neutropeniaAE[, ind_RR]
    cost_params[[country]]$p_qtcfAE[, ind_P] <- cost_params[[country]]$p_qtcfAE[, ind_RR]
    cost_params[[country]]$p_renalAE[, ind_P] <- cost_params[[country]]$p_renalAE[, ind_RR]
    cost_params[[country]]$p_visionAE[, ind_P] <- cost_params[[country]]$p_visionAE[, ind_RR]
    cost_params[[country]]$p_arthralgiaAE[, ind_P] <- cost_params[[country]]$p_arthralgiaAE[, ind_RR]
    cost_params[[country]]$p_neuroshortAE[, ind_P] <- cost_params[[country]]$p_neuroshortAE[, ind_RR]
    cost_params[[country]]$p_neurolongAE[, ind_P] <- cost_params[[country]]$p_neurolongAE[, ind_RR]
    cost_params[[country]]$livertest_sched[ind_P, 1:dur_P] <- cost_params[[country]]$livertest_sched[ind_RR, 1:dur_P]
    cost_params[[country]]$fullblood_sched[ind_P, 1:dur_P] <- cost_params[[country]]$fullblood_sched[ind_RR, 1:dur_P]
    cost_params[[country]]$ecg_sched[ind_P, 1:dur_P] <- cost_params[[country]]$ecg_sched[ind_RR, 1:dur_P]
    cost_params[[country]]$neuroscreen_sched[ind_P, 1:dur_P] <- cost_params[[country]]$neuroscreen_sched[ind_RR, 1:dur_P]
  } else if(analysis=="good_safety") {
    cost_params[[country]]$p_liverAE[, ind_P] <- 0.5*cost_params[[country]]$p_liverAE[, ind_RS]
    cost_params[[country]]$p_pancreasAE[, ind_P] <- 0.5*cost_params[[country]]$p_pancreasAE[, ind_RS]
    cost_params[[country]]$p_anemiaAE[, ind_P] <- 0.5*cost_params[[country]]$p_anemiaAE[, ind_RS]
    cost_params[[country]]$p_neutropeniaAE[, ind_P] <- 0.5*cost_params[[country]]$p_neutropeniaAE[, ind_RS]
    cost_params[[country]]$p_qtcfAE[, ind_P] <- 0.5*cost_params[[country]]$p_qtcfAE[, ind_RS]
    cost_params[[country]]$p_renalAE[, ind_P] <- 0.5*cost_params[[country]]$p_renalAE[, ind_RS]
    cost_params[[country]]$p_visionAE[, ind_P] <- 0.5*cost_params[[country]]$p_visionAE[, ind_RS]
    cost_params[[country]]$p_arthralgiaAE[, ind_P] <- 0.5*cost_params[[country]]$p_arthralgiaAE[, ind_RS]
    cost_params[[country]]$p_neuroshortAE[, ind_P] <- 0.5*cost_params[[country]]$p_neuroshortAE[, ind_RS]
    cost_params[[country]]$p_neurolongAE[, ind_P] <- 0.5*cost_params[[country]]$p_neurolongAE[, ind_RS]
  } else if(analysis=="good_baseres") {
    RR <- rowSums(params[[country]]$pop_start[,c(2,4)])
    if(country=="india") {
      p_NR_RR <- 0.007*0.5
    ...
    RS_NR <- (1-RR)*0.001
    RS_NS <- (1-RR)*(1-0.001)
    params[[country]]$pop_start <- cbind("none"=RS_NS, "R"=RR_NS, "N"=RS_NR, "RN"=RR_NR)
  } else if(analysis=="bad_baseres") {
    RR <- rowSums(params[[country]]$pop_start[,c(2,4)])
    if(country=="india") {
      p_NR_RR <- 0.007*2
    } else if(country=="southafrica") {
    ...
    RS_NS <- (1-RR)*(1-0.004)
    params[[country]]$pop_start <- cbind("none"=RS_NS, "R"=RR_NS, "N"=RS_NR, "RN"=RR_NR)
  } else if(analysis=="good_restrend") {
    params[[country]]$resist_trend[,2] <- params[[country]]$resist_trend[,2]/2
    params[[country]]$resist_trend[,3] <- params[[country]]$resist_trend[,3]/2
  } else if(analysis=="bad_restrend") {
    params[[country]]$resist_trend[,2] <- params[[country]]$resist_trend[,2]*3
    params[[country]]$resist_trend[,3] <- params[[country]]$resist_trend[,3]*3
  }
  
  for(scenario in scenarios) {
    print(scenario)
    if(scenario=="Pan_inject") { #no longer separate LAI costs
    ...
    if(scenario=="Pan_DST") {
      pars$dstR_retx <- rep(1, 10000) #everyone who gets novel DST gets rif DST (otherwise retx leads to less DST)
    }
    ...
    for(j in 1:n) {
      print(j)
      #cat('\r', paste(round(j/n * 100), "% done", sep = " ")) # display the progress of the simulation
      ...
      for(i in 1:10) {
        ...
        #these costs will accrue to retreatments
        ...
        retx_fail <- failures #assume no mortality and all still linked to care
        ...
        dalys <- cbind(dalys, 
                       "dalys_retx"=retx*pars$dalys_tb[j]*0.5 +
                         ...
                         retx*dalys[,"dalys_ae"]) #no post TB because they already accrued it - could consider increasing it though
