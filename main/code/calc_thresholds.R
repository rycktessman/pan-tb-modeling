...
#replace with main analysis SQ if changing resistance trends
if(analysis %in% c("bad_baseres", "good_baseres", "bad_restrend", "good_restrend")) {
  out_sq <- fread(paste0(path_out, "final_outputs_main.csv")) 
  out_all <- out_all %>% filter(scenario!="SQ")
  out_sq <- out_sq %>% filter(scenario=="SQ")
  out_all <- rbind(out_sq, out_all)
}
...
#calculate average (with or without discounting) across years
...
#rest of these only run for main analysis (they go in the appendix)
if(analysis=="main") {
