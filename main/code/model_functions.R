...
calc_pretx <- function(pop, pars, time, index=1, type="new") {
  ...
  if(pars$dstR_after_N) { #only used in Pan scenarios - otherwise needs edits
...
calc_num_tx <- function(startmat, pars) {
  ...
  #number treated, by resistace and regimen (numerator)
...
treatment_model <- function(startmat, pars, index=1) {
  with(pars, {
    ...
    #propogate LTFU by week across each regimen and resistance type
...
#function to replace weekly schedule with another regimen's weekly schedule - used in OWSA
replace_schedule <- function(schedule, ind_original, ind_replacement) {
  schedule[ind_original, ] <- schedule[ind_replacement, ]
  return(schedule)
}
